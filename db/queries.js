...
async function displayEmployees() {
  const [allEmployees, employeesfields] = await connection.promise().query(
    `SELECT 
        ...
        LEFT JOIN departments dep ON roles.department_id = dep.id
...
async function displayRoles() {
  const [roles, rolesFields] = await connection
    .promise()
...
async function displayDepartments() {
  const [departments, departmentsFields] = await connection
    .promise()
...
async function addedEmployee() {
  ...
  await inquirer
    ...
    .then(async (data) => {
      const managerId =
        data.empManager === 'NO MANAGER' ? null : data.empManager;
